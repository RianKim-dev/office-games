...
**Supabase SQL Editor에서 `supabase/schema.sql`을 다시 실행**해야 한다 (여러 번 실행해도 안전). 정리 cron 주기/기준도 함께 바뀌므로 전체 실행을 권장한다. 컬럼만 급하게 맞추려면:
...
0. **위 마이그레이션 실행** — `last_seen_at`이 없으면 목록/방이 동작하지 않는다
